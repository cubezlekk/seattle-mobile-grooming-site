...
const BUSINESS = {
  name: 'Seattle Mobile Grooming Co.',
  tagline: 'A full grooming studio, parked in your driveway',
  phone: '',
  email: 'alissa@seattlemobilegrooming.com',
  address: 'Mobile — serving Seattle & nearby',
  hours: 'By appointment, 7 days a week',
};

const SERVICES = [
  { icon: Sparkles, title: 'Full Groom, At Your Door', desc: 'Bath, cut, and style — done entirely in our self-contained van.' },
  { icon: PawPrint, title: 'Nail Trim & Paw Care', desc: 'Quick, low-stress nail trims without a car ride for your pet.' },
  { icon: Heart, title: 'Senior & Anxious Pets', desc: 'One-on-one attention with no waiting room or other dogs around.' },
  { icon: ShieldCheck, title: 'Cats & Small Dogs', desc: '25 lbs and under — calm, contained, and gentle.' },
  { icon: Truck, title: 'We Come To You', desc: 'No drop-off, no pickup drive — we park right outside.' },
  { icon: Clock, title: 'Flexible Scheduling', desc: '7-day availability built around your routine.' },
];
    ...
    <>
      ...
      >
        <a href="#top" className="flex items-center gap-2 font-display font-bold text-white">
          <span className="w-8 h-8 rounded-lg bg-accent text-white flex items-center justify-center font-extrabold">S</span>
          Seattle Mobile Grooming
        </a>
  ...
  return (
    <header id="top" className="relative min-h-[100dvh] flex items-center overflow-hidden bg-surface text-white">
      <div
        ...
        style={{
          backgroundImage:
            "url('https://images.unsplash.com/photo-1611173622933-91942d394b04?q=80&w=1920&auto=format&fit=crop')",
        }}
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-24">
        <p ref={eyebrowRef} className="eyebrow mb-4">Mobile Studio · Serving Seattle</p>
        <h1 ref={headingRef} className="font-display font-extrabold text-5xl md:text-7xl leading-[1.05] max-w-3xl">
          <span className="block">The grooming studio</span>
          <span className="block italic font-flourish font-medium text-accent">comes to you.</span>
        </h1>
        <p ref={subRef} className="font-body text-white/80 text-lg max-w-xl mt-6">
          One-on-one dog and cat grooming, done entirely in our van, right outside your door.
          No car ride, no waiting room.
        </p>
        <div ref={ctaRef} className="mt-8">
          <a ref={btnRef} href="#contact" className="magnetic-btn">
            Book the Van <Truck size={18} />
          </a>
        </div>
      </div>
    </header>
  ...
  return (
    <section id="features" className="bg-surface text-white py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6">
        <p className="eyebrow mb-3">Why Mobile</p>
              ...
              {Array.from({ length: 7 }).map((_, i) => (
                <PawPrint
                  ...
                  style={{
                    left: `${(i * 41) % 100}%`,
                    top: 0,
                    animation: `rain-fall ${2.7 + (i % 4) * 0.6}s linear ${i * 0.4}s infinite`,
                  }}
                />
              ))}
            </div>
          ...
          <div className="rounded-3xl bg-white/5 border border-white/10 p-8 h-72 relative overflow-hidden">
            ...
            <div className="mt-6 grid grid-cols-4 gap-2">
              {Array.from({ length: 16 }).map((_, i) => (
                <div key={i} className={`h-8 rounded-md ${i === 8 ? 'bg-accent' : 'bg-white/10'}`} />
              ))}
            </div>
            <Truck className="absolute bottom-6 right-6 text-accent animate-pulse" size={28} />
          </div>
        </div>
      </div>
    </section>
  ...
  return (
    <section className="bg-paper text-ink py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6 grid sm:grid-cols-3 gap-10 text-center">
        {[
          { target: 25, suffix: ' lbs & under', label: 'Cats & small dogs' },
          { target: 7, suffix: ' days/week', label: 'Flexible booking' },
          { target: 1, suffix: '-on-1', label: 'Pet at a time' },
        ].map((p) => (
          <div key={p.label}>
...
function Protocol() {
  ...
  const steps = [
    { n: '01', title: 'Book your slot', desc: 'Tell us your address, your pet\'s breed, and any concerns.' },
    { n: '02', title: 'We park outside', desc: 'The van pulls up — no need to load your pet into a car.' },
    { n: '03', title: 'Fresh, happy pet', desc: 'Full groom done in the van, your pet back inside within the hour.' },
  ];
  ...
  return (
    <section id="how-it-works" className="bg-paper text-ink py-24 md:py-32">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <h2 className="font-display text-3xl md:text-5xl font-bold mb-16">Simple, right outside your door.</h2>
...
function TrustSignals() {
  const badges = [
    { icon: Truck, title: 'Fully Self-Contained', desc: 'Water, power, and grooming gear all in the van.' },
    { icon: Heart, title: 'Founded 2009', desc: 'Alissa Rose has been grooming professionally since 2009.' },
    { icon: CheckCircle2, title: 'By Appointment', desc: 'Focused, unhurried time for every pet.' },
  ];
      ...
      <div className="max-w-3xl mx-auto px-6">
        ...
        {status === 'sent' ? (
          ...
          <form onSubmit={submit} className="grid sm:grid-cols-2 gap-4">
            ...
            <input required placeholder="Address / Zip" className="rounded-xl border border-black/10 px-4 py-3 font-body sm:col-span-1" />
            <textarea required placeholder="Pet name, breed, and anything we should know" rows={4} className="rounded-xl border border-black/10 px-4 py-3 font-body sm:col-span-2" />
  ...
  return (
    <footer className="bg-surface text-white py-16">
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-4 gap-10">
        <div>
          <p className="font-display font-bold text-lg mb-2">Seattle Mobile Grooming Co.</p>
      ...
      <p className="text-center text-white/30 text-xs font-mono mt-12">
        © {new Date().getFullYear()} Seattle Mobile Grooming Co. All rights reserved.
      </p>
    </footer>
  );
}
